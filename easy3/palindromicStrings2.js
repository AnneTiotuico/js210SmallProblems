...
function isRealPalindrome(string) {
  let cleanedStr = string.toLowerCase().replace(/[^a-z0-9]/ig, '');
...
The first part of the solution makes all characters lowercased.
The second uses the removeNonLetterNumbers function to replace
any characters that are not alphanumeric, using two helper
functions that check for either a letter or number, respectively.
The solution relies on this to clean up the characters that are
not subject to comparison. It then uses the isPalindrome function
