...
sense. For each roll, only one outcome should be displayed.
...
In switch statements, JavaScript executes the statement associated with
the first case expression that matches. Then, it will continue executing
all subsequent statements, until it comes across a break statement.
Since we did not include any break statements in our original code,
when the value of result is 1, all statements will be executed:
automaticFail(), automaticSuccess(), as well as the default statement.

If this sounds only vaguely familiar, review the assignment on
conditionals or the MDN page on switch.

Further Exploration
This behavior of switch statements might seem strange at first, but there
are scenarios where it's handy to "fall through" to subsequent cases,
for example when you want the same statement to be executed in several cases.
Here is one such example, where our educated role-playing character comments
on the difficulty of a language:
