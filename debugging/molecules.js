...
12 C atoms, 22 H atoms, and 1 O atom, summing up to 12*4 + 22*1 + 1*6 = 76 valence electrons.
...
e.g. valenceOfMolecule('C12', 'H22', 'O'). It then returns the total number of
...
Recall that the arguments object is not an array. It is array-like in the
sense that it implements length and allows you to access its elements by
position (arguments[0], arguments[1], etc.), but it lacks all other
array methods, including forEach.

Since forEach is not defined for arguments, line 14 raises an exception:
TypeError: arguments.forEach is not a function

In order to be able to use forEach, we must call it on an actual array.
Unlike the arguments object, rest parameters provide an actual array of arguments.
