...
One function signature expects two arguments, and one expects only a single
argument. But JavaScript does not support function overloading
(the ability to utilize multiple functions of the same name with different
signatures). So with the second definition of range, the first one is
overridden. That is, it is always range(end) on lines 11-13 that is executed,
no matter how many arguments you provide. So when we call range(10, 20) on
line 18, the parameter end is assigned to 10, and the second argument, 20,
is ignored. The function then executes its body, line 12, calling itself again,
this time with two arguments, 0 and 10. Since our program only recognizes the
