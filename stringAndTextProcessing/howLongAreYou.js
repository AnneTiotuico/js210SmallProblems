...
function wordLengths(str) {
  if (!str) return [];
  return str.split(' ').flatMap(word => [word, word.length].join(' '));
}

// function wordLengths(str) {
//   return str ? str.split(' ').flatMap(word => [word, word.length].join(' ')) : [];
// }
...
It converts the input string into an array of words using a space (' ')
as a separator. The solution then uses the Array.prototype.map method to
transform each word in the array into a string containing the word followed
by a space and the word's length.

The solution uses a guard clause to handle the cases in which the argument
is an empty string or no argument is passed. For these cases, the solution
