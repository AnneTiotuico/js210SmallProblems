...
//   return [...string].reduce((result, char, idx) => {
//     return result.concat(leadingSubstrings(string.slice(idx)));
//   }, []);
// }

// function substrings(string) {
//   return [...string].map((_, idx) => leadingSubstrings(string.slice(idx))).flat();
// }
...
Even with the help of the leadingSubstrings function written earlier,
this problem still seems to be easiest when we use an ordinary for loop.
The twist here is that substrings handles the process of finding the substrings
on the trailing end of the original string, while leadingSubstrings handles
the leading substrings.

If that's a little hard to hold in your head, think about what must happen here.
We'll assume that the original string is abc:
...
Further Exploration
Rewrite substrings using list processing functions. That is, convert the string
into an array of some sort and use functions like map, filter, or reduce to get
the desired substrings. (You will also need to use join.)
