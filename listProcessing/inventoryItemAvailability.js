...
function isItemAvailable(itemID, transactions) {
  let remainingQty = transactionsFor(itemID, transactions).reduce((qty, tr) => {
     return qty + (tr.movement === 'in' ?  tr.quantity : -tr.quantity);
  }, 0);
...
list containing only the transactions related to the specified inventory item.
The solution then computes the sum by using a reducing strategy.

The callback function passed to the Array.prototype.reduce method takes
two arguments: a sum integer and a transaction object. The solution
initializes sum to 0 because if it weren't it would be implicitly set to
the first element, which is an object
(i.e., { id: 101, movement: 'in', quantity: 5 }).
Consequently, the return value of the callback won't be an accumulation of
the quantity values. For each transaction object, if the value of movement
is 'in', the solution increments sum by the value of transaction.quantity;
otherwise, sum is decremented by that value.

The solution stores the result of this reduction in the quantity variable.
If quantity is greater than 0, the isItemAvailable function will return true;
